...
import com.llyycci.void_power.network.SP.SP_HologramPoseUpdate;

import org.jetbrains.annotations.Nullable;

import com.llyycci.void_power.config.ModConfigs;
import com.llyycci.void_power.utils.ParamUtils;
import com.llyycci.void_power.utils.font.DefaultFont;
import com.llyycci.void_power.utils.font.Font;
import com.llyycci.void_power.utils.font.FontLib;
import com.llyycci.void_power.utils.font.IFontLib;
import com.llyycci.void_power.world.blocks.hologram.DefaultFrameBufferImpl;
import com.llyycci.void_power.world.blocks.hologram.HologramTE;
import com.llyycci.void_power.world.blocks.hologram.IFrameBuffer;
...
public class P_HologramPeripheral extends ModSyncedPeripheral<HologramTE>{
...
//		buffers = new IFrameBuffer[ModConfigs.common().HOLOGRAM_BUFFER_COUNT.get()];
//        Arrays.fill(buffers, null);
//    }
	public P_HologramPeripheral(HologramTE be){
		super(be);
		currentBuffer = be;
		buffers = new IFrameBuffer[ModConfigs.common().HOLOGRAM_BUFFER_COUNT.get()];
		Arrays.fill(buffers, null);
    ...
    public final int CreateFrameBuffer(int w, int h) throws LuaException {
        int idx = -1;
//		VoidPowerMod.LOGGER.info("HOLOGRAM_CreateFrameBuffer: {},{}", w, h);
//        if(w > Config.holo_w_mx*2 || h > Config.holo_h_mx*2) throw new LuaException("Max resolution is %d x %d, out of range.".formatted(Config.holo_w_mx*2, Config.holo_h_mx*2));
		if(w > ModConfigs.common().HOLOGRAM_RES_X_MAX.get() ||  h > ModConfigs.common().HOLOGRAM_RES_Y_MAX.get()){
			throw new LuaException("Max resolution is %d x %d, out of range.".formatted(ModConfigs.common().HOLOGRAM_RES_X_MAX.get(), ModConfigs.common().HOLOGRAM_RES_Y_MAX.get()));
		}
    ...
    public final void FreeFrameBuffer(int idx) throws LuaException {
        if(idx >= 0 && idx < buffers.length){
            if(currentBuffer == buffers[idx]) currentBuffer = blockEntity;
            buffers[idx] = null;
        }
...
//		VoidPowerMod.LOGGER.info("CurrentSize: {},{}", currentBuffer.getWidth(), currentBuffer.getHeight());
//		VoidPowerMod.LOGGER.info("HOLOGRAM_Resize: {},{}", w, h);
...
//			currentBuffer = currentBuffer.resize(w, h);
			// 关键修改：直接调用blockEntity的resize方法
			blockEntity.resize(w, h);

			// 如果currentBuffer不是blockEntity，则也需要更新
//			if (currentBuffer != blockEntity) {
//				currentBuffer = currentBuffer.resize(w, h);
//			} else {
//				// 确保currentBuffer指向blockEntity
//				currentBuffer = blockEntity;
//			}
//
			// 添加以下代码触发同步更新
			blockEntity.fullSync.set(true);     // 设置需要全量同步
			blockEntity.transformDirty.set(true); // 设置变换矩阵需要更新
			blockEntity.force_full_sync_ticker = 0;// 重置全量同步计时器
//
			// 立即执行同步，不等待下一个tick
			blockEntity.FullSyncPack();
			PacketManager.sendToAllPlayerTrackingThisBlock(new SP_HologramPoseUpdate(blockEntity), blockEntity);
        }
    }
    ...
    public final void DrawLine(IArguments param) throws LuaException {
        int r_ma_x = blockEntity.getWidth()*2;
        int r_mi_x = -blockEntity.getWidth();
        int r_ma_y = blockEntity.getHeight()*2;
        int r_mi_y = -blockEntity.getHeight();
...
//		}
    ...
    public final void DrawTriangleWithTexture(IArguments param) throws LuaException {
        ...
        if(texture == null){
            throw new LuaException("Frame buffer not allocablockEntity.");
        }
...
//		VoidPowerMod.LOGGER.info("Text: {}", text);
...
//				VoidPowerMod.LOGGER.info("ch: {}", ch);
                Font.CharMat cm = font.get(ch);
//				VoidPowerMod.LOGGER.info("cm_bitmap_length: {}", cm.bitmap.length);
...
//						VoidPowerMod.LOGGER.info("cm.bitmap[{}][{}]:{}", y-ay, x-tmpx, cm.bitmap[y-ay][x-tmpx]);
...
//							VoidPowerMod.LOGGER.info("color_blend: {} + {} = {}", color, ParamUtils.convertColor(buffer[offD + x]), buffer[offD + x]);
						}
						else {
							buffer[offD + x] = ParamUtils.convertColor(ParamUtils.blendColor(0x00000000, ParamUtils.convertColor(buffer[offD + x])));
                        }
                    }
                }
                tmpx += cm.width;
		...
		// 标记需要同步
		blockEntity.needSync.set(true);
		// 如果文本较长或者是关键文本，设置fullSync
		if (text.length() > 10) {
			blockEntity.fullSync.set(true);
		}
		//同步标记
    ...
    public final void SetScale(double x, double y) throws LuaException {
        ...
        synchronized (SYNC_LOCK) {
            blockEntity.scalex = (float) x;
            blockEntity.scaley = (float) y;
        }
        blockEntity.transformDirty.set(true);
    }
...
//				VoidPowerMod.LOGGER.info("Full Sync");
//                blockEntity.fullSync.set(true);
//                return;
//            }
//        }
//        blockEntity.needSync.set(true);
		boolean fullSync = param.count() > 0 && param.getBoolean(0);

		// 如果当前缓冲区不是主缓冲区，先将内容复制到主缓冲区
		if (currentBuffer != blockEntity) {
			VoidPowerMod.LOGGER.info("Syncing custom buffer to main buffer");
			int[] srcBuffer = currentBuffer.getBuffer();
			int[] destBuffer = blockEntity.getBuffer();
			int srcWidth = currentBuffer.getWidth();
			int srcHeight = currentBuffer.getHeight();
			int destWidth = blockEntity.getWidth();
			int destHeight = blockEntity.getHeight();

			// 只复制重叠的区域
			int minWidth = Math.min(srcWidth, destWidth);
			int minHeight = Math.min(srcHeight, destHeight);

			for (int y = 0; y < minHeight; y++) {
				System.arraycopy(srcBuffer, y * srcWidth, destBuffer, y * destWidth, minWidth);
			}
		}
...
//
//			// 创建临时副本，用于操作
//			int[] destBuffer = blockEntity.getBuffer().clone();
//			int destWidth = blockEntity.getWidth();
//			int destHeight = blockEntity.getHeight();
//
//			// 先用主缓冲区的初始颜色填充整个缓冲区
//			int initColor = blockEntity.getInitColor();
//			Arrays.fill(destBuffer, initColor);
//
//			// 然后只复制自定义缓冲区中实际绘制的内容
//			int[] srcBuffer = currentBuffer.getBuffer();
//			int srcWidth = currentBuffer.getWidth();
...
//					int srcColor = srcBuffer[y * srcWidth + x];
//					if (srcColor != currentBuffer.getInitColor()) {
//						destBuffer[y * destWidth + x] = srcColor;
//					}
//				}
//			}
//
//			// 将处理后的缓冲区复制回主缓冲区
//			System.arraycopy(destBuffer, 0, blockEntity.getBuffer(), 0, destBuffer.length);
//		}
		// 设置同步标记
		if (fullSync) {
//			VoidPowerMod.LOGGER.info("Full Sync Requested");
			blockEntity.fullSync.set(true);
//			blockEntity.force_full_sync_ticker = 0;
			blockEntity.FullSyncPack();
//			blockEntity.transformDirty.set(true);
			PacketManager.sendToAllPlayerTrackingThisBlock(new SP_HologramPoseUpdate(blockEntity), blockEntity);
		} else {
//			VoidPowerMod.LOGGER.info("Partial Sync Requested");
			blockEntity.needSync.set(true);
			blockEntity.NoFullSyncPack();
			PacketManager.sendToAllPlayerTrackingThisBlock(new SP_HologramPoseUpdate(blockEntity), blockEntity);
		}
    }
    ...
    public final void SetRotation(double yaw, double pitch, double roll){
        synchronized (SYNC_LOCK) {
            blockEntity.rotYaw = (float) yaw;
            blockEntity.rotPitch = (float) pitch;
            blockEntity.rotRoll = (float) roll;
        }
        blockEntity.transformDirty.set(true);
    }
    ...
    public final void SetTranslation(double x, double y, double z) throws LuaException {
        ...
        synchronized (SYNC_LOCK) {
            blockEntity.offx = (float) x;
            blockEntity.offy = (float) y;
            blockEntity.offz = (float) z;
        }
        blockEntity.transformDirty.set(true);
    }
    ...
    public final String CreateFont(String name) throws LuaException {
        ...
        if(fontlibs.containsKey(name)){
            ...
            if(fontlibs.size() < ModConfigs.common().HOLOGRAM_FONT_COUNT.get()){
